...

{
    ...
    \block[bodyverticalshift=0cm, bodyinnersep=3mm]{}{
        \centering\begin{minipage}{0.94\textwidth}
            \textbf{Graphs} allow us to model complex and dynamic relationships, and can be used to
...
\begin{wrapfigure}[11]{r}{0.4\linewidth}
    ...
    \begin{center}\begin{tikzpicture}[scale=1.1]%{{{
        ...
        {\vphantom{0}};
        \node[draw, circle, fill=uofgpumpkin, inner sep=5pt, font=\bfseries] (Nc) at (0, -4)
        {\vphantom{0}};
        \node[draw, circle, fill=uofgpumpkin, inner sep=5pt, font=\bfseries] (Nd) at (2, -4)
        {\vphantom{0}};
...
\block{Parallel Search: Irregularity, Heuristics, and Diversity}{
    ...
    \begin{tikzpicture}[scale=2.5]%{{{
        ...
        \tikzstyle{p} = [draw, rounded corners, color=uofgrose, fill=uofgrose!50, line width=1pt];
    ...
    In practice, different work splitting methods give vastly different performance:
...
\begin{wrapfigure}[20]{r}{0.68\linewidth}\begin{center}
    \small\input{gen-graph-really-hard}
\end{center}\end{wrapfigure}

    Modern algorithms can quickly solve nearly every subgraph isomorphism problem instance from
    standard benchmark suites involving patterns with up to \textbf{a thousand vertices}, and
    targets with up to \textbf{ten thousand vertices}. From this, an optimist might conclude that
    subgraph isomorphism is easy in practice.

    \medskip

    We show how to generate instances with patterns with between ten and thirty vertices, and
    targets with one hundred and fifty vertices, that cannot be solved within an hour.

    \medskip

    Phase transition in random graphs, which we can locate reasonably accurately. Difficult central
    region. Constrainedness.

    \medskip

    Phase transition in random graphs, which we can locate reasonably accurately. Difficult central
    region. Constrainedness. \[ \mathsf{\langle Sol \rangle = t^{\underline{p}} \cdot {d_t}^{d_p \cdot
    \binom{p}{2}} \cdot
    {(1 - d_{t})}^{(1 - d_{p}) \cdot \binom{p}{2}}} \] things.
